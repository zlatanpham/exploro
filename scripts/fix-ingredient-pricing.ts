  ...
  try {
    ...
    for (const fix of PRICING_FIXES) {
      ...
      if (ingredient) {
        ...
        if (oldPrice && oldPrice > 1000000) {
          await prisma.ingredient.update({
            where: { id: ingredient.id },
            data: { current_price: fix.reasonable_price }
          });

          console.log(`✅ Fixed "${ingredient.name_vi}": ${oldPrice.toLocaleString()}₫ → ${fix.reasonable_price.toLocaleString()}₫ per ${ingredient.unit?.symbol || 'unit'}`);
          fixedCount++;
        } else {
          console.log(`ℹ️  "${ingredient.name_vi}" already has reasonable pricing: ${oldPrice?.toLocaleString() || 'N/A'}₫ per ${ingredient.unit?.symbol || 'unit'}`);
        }
      } else {
    ...
    if (expensiveIngredients.length > 0) {
      ...
      expensiveIngredients.forEach(ing => {
        console.log(`  - "${ing.name_vi}": ${ing.current_price?.toLocaleString()}₫ per ${ing.unit?.symbol || 'unit'}`);
      });
    } else {
    ...
    console.log(`  Ingredients with high prices requiring review: ${expensiveIngredients.length}`);
