...
async function debugMuongConversion() {
  ...
  try {
    ...
    // Get units
    const muongUnit = await prisma.unit.findUnique({
      where: { symbol: "muỗng" },
    });
    ...
    if (tieuIngredient) {
      ...
      console.log(
        `Price: ${tieuIngredient.current_price?.toLocaleString()}₫ per ${tieuIngredient.unit?.symbol}`,
      );
      ...
      const dishIngredients = await prisma.dishIngredient.findMany({
        ...
        include: {
          ...
          unit_ref: { include: { category: true } },
        },
      });
      ...
      dishIngredients.forEach((di) => {
        ...
        console.log(
          `  Recipe: ${di.quantity.toString()} ${di.unit_ref?.symbol} (${di.unit_ref?.category?.name})`,
        );
        console.log(`  Base unit: ${di.ingredient.unit?.symbol}`);
        console.log(
          `  Converted quantity: ${di.converted_quantity?.toString() || "none"}`,
        );
        console.log(
          `  Price: ${di.ingredient.current_price?.toString()}₫ per ${di.ingredient.unit?.symbol}`,
        );

        if (di.converted_quantity) {
          const conversionRatio =
            Number(di.converted_quantity) / Number(di.quantity);
          console.log(
            `  Conversion ratio: 1 muỗng = ${conversionRatio}${di.ingredient.unit?.symbol}`,
          );

          if (conversionRatio > 1000) {
            console.log(
              `  ❌ PROBLEM: Ratio is too high! Should be ~15 for grams`,
            );
          }

          const price =
            Number(di.converted_quantity) * Number(di.ingredient.current_price);
    ...
    allMuongIngredients.forEach((di) => {
      const conversionRatio = di.converted_quantity
        ? Number(di.converted_quantity) / Number(di.quantity)
        : 0;
      console.log(
        `  - ${di.ingredient.name_vi}: ${di.quantity.toString()} muỗng → ${di.converted_quantity?.toString() || "none"} ${di.ingredient.unit?.symbol} (ratio: ${conversionRatio})`,
      );
    });
  } catch (error) {
